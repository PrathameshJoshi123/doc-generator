...
export const MarkdownPreview: React.FC<MarkdownPreviewProps> = ({
  ...
  const components = {
    ...
    p: ({ children, ...props }) => (
      <p
        className="mb-3 sm:mb-4 text-gray-200 leading-relaxed text-sm sm:text-base lg:text-lg text-left"
        {...props}
      >
        {children}
      </p>
    ...
    li: ({ children, ...props }) => (
      <li
        className="text-gray-200 text-sm sm:text-base lg:text-lg group flex items-start"
        {...props}
      >
    ...
    hr: ({ ...props }) => (
      <hr
        className="my-6 sm:my-8 border-0 h-px bg-gradient-to-r from-transparent via-cyan-500/50 to-transparent"
        {...props}
      />
    ),
  ...
  useEffect(()=>{
    console.log("inside useEffect", repo)
  },[repo])
  return (
