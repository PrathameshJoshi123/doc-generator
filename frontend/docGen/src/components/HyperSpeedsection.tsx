...
const HyperspeedSection: React.FC = () => {
  ...
  return (
    <section className="py-8 xs:py-12 sm:py-16 lg:py-20 px-1 xs:px-2 sm:px-4 md:px-6 lg:px-8 bg-gradient-to-br from-gray-800 via-gray-900 to-gray-800">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-4 xs:mb-6 sm:mb-8 md:mb-12">
          <h2 className="text-xl xs:text-2xl sm:text-3xl lg:text-4xl font-bold mb-2 xs:mb-3 sm:mb-4 text-white px-1 xs:px-2">
            Lightning Fast Processing
          </h2>
          <p className="text-sm xs:text-base sm:text-lg md:text-xl text-gray-300 max-w-3xl mx-auto px-2 xs:px-4 leading-relaxed">
        ...
        <div className="mt-4 xs:mt-6 sm:mt-8 md:mt-12 grid grid-cols-1 sm:grid-cols-3 gap-3 xs:gap-4 sm:gap-6 md:gap-8 text-center px-1 xs:px-2">
          {stats.map((stat, index) => (
            <div key={index} className="bg-gray-800 rounded-lg xs:rounded-xl p-3 xs:p-4 sm:p-6 shadow-sm border border-gray-700">
              <div className="text-lg xs:text-xl sm:text-2xl md:text-3xl font-bold text-blue-400 mb-1 xs:mb-2">
                {stat.value}
              </div>
              <div className="text-xs xs:text-sm sm:text-base text-gray-300">
                {stat.label}
              </div>
            </div>
          ))}
